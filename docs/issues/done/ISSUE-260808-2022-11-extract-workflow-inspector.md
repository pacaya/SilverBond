...
- [ ] The generic merge helper is importable from a plain module rather than a component instance script. After the change, `rg -n 'mergeConfig' ui/src/features/editor/` shows it defined and exported from a `.ts` module, with the parent importing rather than defining it; before this change the only definition is inside the parent's instance script. Whatever declaration form the lift uses, **export it under the name `mergeConfig`** — `ISSUE-260723-0823-01` runs a pre-flight check on that name and would read false otherwise.
...
- Ownership of the `mergeConfig` lift belongs to this record (gate round 1/2 resolution), because the run-as helpers cannot move without it; `ISSUE-260723-0823-01` was reopened and states the module already exists at its baseline. Export name must literally be `mergeConfig` — that sibling pre-flight-checks the name; declaration form is free.
- The kind-panels record (`ISSUE-260723-0823-01`) later extends the same module with per-kind accessors/defaults and owns the `InspectorPanel.svelte:1` Divergent Change ledger row; close no row here.
...
**Full artifacts:** docs/issues/ISSUE-260808-2022-11-extract-workflow-inspector.md (Agent Brief + Triage Notes) · sibling records `ISSUE-260723-0823-01`, `ISSUE-260808-2022-04` under docs/issues/ · CLAUDE.md
...
**Class 3 — the merge helper is a third coupling, unnamed, and collides with a sibling.** The run-as helpers this brief tells the agent to move are written in terms of a generic merge helper defined in the parent's instance script, whose remaining call sites stay in the parent. A component instance script is not importable — the brief's own argument for the access-modes helper — so the run-as helpers cannot travel as written, and the brief said nothing. Worse, lifting it is claimed as a deliverable by `ISSUE-260723-0823-01`, which is `blocked_by` this record and therefore lands afterwards; its criterion asserting the helper is defined only inside the parent beforehand would already be green at its own baseline.
...
- **Both this record and `ISSUE-260723-0823-01` probe the lifted helper with the literal `function mergeConfig`.** An arrow-const lift would satisfy neither, and would leave the sibling's pre-flight check reading false.
...
This is the same failure `ISSUE-260723-0823-01` hit at its own round 3, and the same remedy. Recording the earned `PASS` and withdrawing it explicitly is preferred over not writing it, so the round's findings stay in the record and the supersession is auditable.
...
- **No accepted-risk statement for the uncovered sections.** Rendered-output preservation for the Workflow fields and the Variables section rests on review rather than any mechanical guard. `ISSUE-260723-0823-01` records the equivalent exposure as an explicit accepted-risk paragraph; this record leaves it implicit. The most consequential of these notes.
