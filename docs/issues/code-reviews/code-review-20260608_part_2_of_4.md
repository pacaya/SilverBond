...
### F1. `runAs` sandbox bypass in decide/orchestrator one-shots (FINALIZED) (FIXED)
...
### F2. Missing escalation channel auto-approves permission walls (fail-open) (FINALIZED) (FIXED)
...
### F3. Run-level interaction slot race (FINALIZED) (FIXED)
...
### F5. Agent safety config silently dropped on the tmux path (FINALIZED) (FIXED)
...
### F9. `extract_after_prompt` corrupts decide outcomes for multi-line/TUI prompts (FINALIZED) (FIXED)
...
### F4. Interaction-event TOCTOU (emit before register) (FINALIZED) (FIXED)
...
### F6. Destructive blocklist only scans the capture delta (FINALIZED) (FIXED)
...
### F7. Visual idle can end active work prematurely (FINALIZED) (FIXED)
...
### F8. `wrap_keep_open` cannot keep the pane open (FINALIZED) (FIXED)
...
### F10. Blocking login-shell subprocess runs on the async executor (FINALIZED) (FIXED)
